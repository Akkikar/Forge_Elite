    ...
    try {
      ...
      if (!result.match) {
        setFaceMismatchCount((prev) => {
          ...
          if (newCount >= 5) {
            alert("🚫 Face mismatch limit exceeded. Logging out after evaluation.");
  
            // Evaluate as cheating
        ...
        setFaceMismatchCount(0); // Reset count if face matches
      }
    } catch (err) {
  ...
  const detectDevices = useCallback(async (imageData, uid) => {
    try {
      ...
      if (result.action === "logout") {
        ...
        alert(`🚫 Device detected (${result.devices.join(", ")}). Logging out.`);
  ...
  useEffect(() => {
    ...
    if (questions.length === 0) {
      fetch("http://localhost:8000/api/tests/questions/mixed")
        .then((res) => res.json())
        .then((data) => {
          setQuestions(data);  // Only set the questions once
          setLoading(false);
        })
        .catch((err) => {
    ...
    try {
      ...
      stopWebcam();
        if (data.status.passed) {
          alert(`✅ Passed with ${data.status.score}%`);
          stopWebcam();
          navigate("/home");
        } else {
          alert(`❌ Failed with ${data.status.score}%. Retake after 7 days.`);
          stopWebcam();
          navigate("/reference");
        }
    } catch (err) {
