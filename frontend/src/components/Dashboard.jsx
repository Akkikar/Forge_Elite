  ...
  useEffect(() => {
    const fetchSelfieStatus = async () => {
      ...
      try {
        ...
        setError("There was an error checking selfie status.");
      }
    };
  ...
  const submitSelfie = async () => {
    ...
    try {
      ...
      if (data.status === "Valid face detected. No external devices found.") {
        ...
        alert("🚫 Device Detected: " + data.devices.join(", ") + ". Submission blocked.");
      } else {
        alert("❌ Failed to upload selfie. Reason: " + (data.error || "Unknown error"));
      }

    } catch (err) {
      console.error("Error uploading selfie:", err);
      alert("❌ Error submitting selfie. Try again.");
    } finally {
      setLoading(false);
    }
  };
